...
from adversarial import FGM_paddle
...
parser.add_argument("--fgm_on", action='store_true')
parser.add_argument("--adv_eps", default=1.0, type=float )
...
def evaluate(model, criterion, metric, data_loader):
    ...
    for batch in data_loader:
        ...
        logits, _ = model(input_ids=input_ids,
                          token_type_ids=token_type_ids, do_evaluate=True)
...
def do_train():
    ...
    print("train set size:", len(train_data_loader))
    print("dev set size:", len(dev_data_loader))
    ...
    if hasattr(args, 'fgm_on') and args.fgm_on:
        fgm = FGM_paddle(model, epsilon=args.adv_eps,
    ...
    for epoch in range(1, args.epochs + 1):
        for step, batch in enumerate(train_data_loader, start=1):
            ...
            if fgm:
                fgm.attack()
                logits1_adv, kl_loss_adv = model(
                    input_ids=input_ids, token_type_ids=token_type_ids)
                ce_loss_adv = criterion(logits1_adv, labels)
                if kl_loss_adv > 0:
