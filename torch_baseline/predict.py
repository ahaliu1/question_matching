...
LastEditTime: 2021-11-20 18:21:03
'''
#!/usr/bin/python
# -*- coding: utf-8 -*-
...
def read_examples(dir, split='train'):
    ...
    for file_name in os.listdir(dir):
        if split == 'train':
            example = read_data(os.path.join(dir, file_name))
...
def get_feature(examples,path):
    # 将feature保存避免耗时二次处理
    if os.path.exists(path):
        candidate_features = pickle.load(open(path, 'rb'))
    else:
        candidate_features = convert_examples_to_features(examples, tokenizer, max_seq_length, True)
        pickle.dump(candidate_features, open(path, 'wb'))
...
print("test feature load finish")
...
for fold, (train_index, valid_index) in enumerate(skf.split(all_label, all_label)):
    ...
    # 得到一折模型对测试集的预测结果
    model.load_state_dict(torch.load('./model_save/1_model/ernie_' + file_name + '_{}.bin'.format(fold)))
    test_preds_fold = np.zeros((len(test_examples), 2))
    model.eval()
    with torch.no_grad():
        for i, batch in tqdm(enumerate(test_loader)):
